...
export default function HomeScreen() {
  ...
  const enrichProjectsWithUser = useCallback(async (projectList) => {
    ...
    return projectList.map((proj) => ({
      ...proj,
      username: usersData[proj.userId]?.username || t("user"),
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (error) => {
        ...
        Alert.alert(t("error"), t("projectsLoadError"));
        setLoading(false);
      }
    );

    return () => unsubscribe();
  }, [enrichProjectsWithUser, t]);
  ...
  useEffect(() => {
    ...
    const fetchFilteredProjects = async () => {
      ...
      try {
        ...
        Alert.alert(t("error"), t("filteredProjectsLoadError"));
      } finally {
        setLoading(false);
      }
    };
  ...
  }, [filterPostal, enrichProjectsWithUser, t]);
