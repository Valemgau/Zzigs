  ...
  const handleUpdateStatus = async (offerId, newStatus, offer) => {
    try {
      ...
      if (newStatus === "confirmed") {
        msgCouturier = {
          title: "Félicitations ! 🎉",
          desc: `Votre offre de ${offer.price}€ pour "${
            offer.project.title || "le projet"
          }" a été acceptée par le client. Vous allez recevoir une proposition de rendez-vous.`,
          type: "offer_accepted",
        };
        msgClient = {
          title: "Offre acceptée",
          desc: `Vous avez accepté l'offre de ${offer.price}€ de ${
            offer.username || "ce couturier"
          }. Vous pouvez maintenant proposer un rendez-vous.`,
          type: "offer_accepted",
        };
      } else if (newStatus === "refused") {
        msgCouturier = {
          title: "Offre déclinée",
          desc: `Votre offre pour "${
            offer.project.title || "le projet"
          }" n'a pas été retenue par le client.`,
          type: "offer_refused",
        };
        msgClient = {
          title: "Offre refusée",
          desc: `Vous avez refusé l'offre de ${
            offer.username || "ce couturier"
          }.`,
          type: "offer_refused",
        };
      ...
      showMessage({
        message: newStatus === "confirmed" ? "Offre acceptée" : "Offre refusée",
        description:
          newStatus === "confirmed"
            ? "Le couturier a été notifié. Proposez-lui un rendez-vous."
            : "Le couturier a été informé de votre décision",
        type: "success",
        icon: "success",
      });
    } catch (error) {
  ...
  const handleConfirmAppointment = async () => {
    ...
    try {
      ...
      const msgCouturier = {
        ...
        desc: `Le client vous propose un rendez-vous le ${moment(
          `${selectedDate}T${selectedTime}`
        ).format("dddd D MMMM à HH:mm")} pour "${
          selectedOffer.project.title || "le projet"
        }". Acceptez ou refusez depuis l'application.`,
      ...
      showMessage({
        message: "Rendez-vous proposé",
        description:
          "Le couturier va recevoir votre proposition et vous répondra rapidement",
        type: "success",
        icon: "success",
      });
  ...
  const handleCancelAppointment = async () => {
    if (!cancelReason.trim()) {
      showMessage({
        ...
        description: "Veuillez indiquer pourquoi vous annulez ce rendez-vous",
        type: "warning",
      });
      return;
    }
    ...
    try {
      ...
      const msgCouturier = {
        ...
        desc: `Le client a annulé le rendez-vous pour "${
          offer?.project.title || "le projet"
        }". Raison : ${cancelReason.trim()}`,
      ...
      showMessage({
        ...
        description: "Le couturier a été informé de l'annulation",
        type: "success",
      });
            ...
            {appointment.status === "pending" && (
              <View className="bg-blue-50 border-l-4 border-blue-500 p-4 mb-3">
                ...
                <Text
                  ...
                  style={{ fontFamily: "OpenSans_700Bold" }}
                >
                  à {moment(appointment.date).format("HH:mm")}
                </Text>
                <Text
                  className="text-xs text-blue-700 mt-2"
                  style={{ fontFamily: "OpenSans_400Regular" }}
                >
                  Le couturier doit accepter ce créneau. Vous serez notifié de
                  sa réponse.
                </Text>
              </View>
            )}
            ...
            {appointment.status === "confirmed" && (
              <View className="bg-green-50 border-l-4 border-green-500 p-4 mb-3">
                ...
                <Text
                  ...
                  style={{ fontFamily: "OpenSans_400Regular" }}
                >
                  Le couturier vous attend ! Consultez l'adresse ci-dessus.
                </Text>
              </View>
            )}
            ...
            {appointment.status === "inProgress" && (
              <View className="bg-orange-50 border-l-4 border-orange-500 p-4 mb-3">
                ...
                <Text
                  ...
                  style={{ fontFamily: "OpenSans_400Regular" }}
                >
                  Le couturier travaille sur votre projet. Vous serez notifié
              ...
              <View className="bg-purple-50 border-l-4 border-purple-500 p-4 mb-3">
                <View className="flex-row items-center mb-2">
                  ...
                  <Text
                    ...
                    style={{ fontFamily: "OpenSans_700Bold" }}
                  >
                    Mission terminée - Paiement requis
                  </Text>
                </View>
                <Text
                  ...
                  style={{ fontFamily: "OpenSans_400Regular" }}
                >
                  Le couturier a marqué la mission comme terminée. Veuillez
                  effectuer le paiement de {item.price}€ pour la prestation via
                  l'application par carte bancaire. Des frais de service
                  supplémentaires seront appliqués au montant final. N'effectuez
                  aucun paiement en dehors de la plateforme.
                </Text>
                <View className="bg-purple-100 p-3">
                  <Text
                    ...
                    style={{ fontFamily: "OpenSans_700Bold" }}
                  >
                    ⚠️ Attention : Le paiement doit obligatoirement se faire
                    dans l'application pour votre sécurité.
                  </Text>
                </View>
              </View>
            )}
            ...
            {appointment.status === "paymentConfirmed" && (
              <View className="bg-green-50 border-l-4 border-green-500 p-4 mb-3">
                ...
                <Text
                  ...
                  style={{ fontFamily: "OpenSans_400Regular" }}
                >
                  Merci d'avoir utilisé zZigs ! N'hésitez pas à laisser un avis.
                </Text>
              </View>
            )}
          ...
          >
            <View style={{ padding: 10 }} className="bg-white p-6">
              ...
              <TextInput
                ...
                placeholder="Ex: Changement de plan, projet reporté..."
